...
func TestLoadParsesAdminTelegramIDs(t *testing.T) {
	...
	t.Setenv("STEAM_BASE_URL", "https://api.steampowered.com")
	t.Setenv("STEAM_WEB_API_KEY", "")
	t.Setenv("DOTA_SYNC_INTERVAL_SECONDS", "30")
	t.Setenv("DOTA_MATCHES_REQUESTED", "7")
	...
	if cfg.Dota.MatchesRequested != 7 {
		t.Fatalf("Dota.MatchesRequested = %d", cfg.Dota.MatchesRequested)
	}
}

func TestLoadParsesSteamProvider(t *testing.T) {
	t.Setenv("APP_ENV", "test")
	t.Setenv("TELEGRAM_BOT_TOKEN", "test-token")
	t.Setenv("DATABASE_URL", "")
	t.Setenv("POSTGRES_HOST", "postgres")
	...
	if cfg.Dota.Provider != "steam" {
		t.Fatalf("Dota.Provider = %q", cfg.Dota.Provider)
	}
	if cfg.Dota.SteamWebAPIKey != "steam-key" {
		t.Fatalf("Dota.SteamWebAPIKey = %q", cfg.Dota.SteamWebAPIKey)
	}
}
